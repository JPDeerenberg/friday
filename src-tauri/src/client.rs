...
// Token shape + expiry math live in magister-core (shared with web-api).
// Re-exported here so every existing `crate::client::TokenSet` path —
// including struct-literal construction in tests — keeps working unchanged.
pub use magister_core::tokens::TokenSet;
