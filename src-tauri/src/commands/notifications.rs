...
#[tauri::command]
pub fn get_relevance_threshold(app: AppHandle) -> Result<i32, String> {
    let _ = &app;
    #[cfg(target_os = "android")]
    {
        let window = app.get_webview_window("main")
        ...
        window.with_webview(move |webview| {
            #[cfg(target_os = "android")]
            {
                let _ = webview.jni_handle().exec(move |env, activity, _webview| {
                    ...
                    let mut result = 30; // default threshold

                    if let Ok(jni::objects::JValueGen::Int(val)) = res {
                        result = val;
                    } else if let Err(_) = res {
                        if let Ok(true) = env.exception_check() {
                            let _ = env.exception_clear();
...
#[tauri::command]
pub fn set_relevance_threshold(app: AppHandle, threshold: i32) -> Result<String, String> {
    let _ = &app;
    let clamped = threshold.clamp(0, 100);
    #[cfg(target_os = "android")]
    {
        let window = app.get_webview_window("main")
            .or_else(|| app.webview_windows().values().next().cloned())
            ...
            #[cfg(target_os = "android")]
            {
                let _ = webview.jni_handle().exec(move |env, activity, _webview| {
                    let res = env.call_method(
                        ...
                        "(I)V",
                        &[jni::objects::JValue::Int(clamped)],
                    );
                    if let Err(_) = res {
                        if let Ok(true) = env.exception_check() {
                            let _ = env.exception_clear();
        ...
        return Ok(format!("Relevance threshold set to {}", clamped));
    }
    #[cfg(not(target_os = "android"))]
    Ok(format!("set_relevance_threshold({}) is only supported on Android", clamped))
}

/// Get the notification history for debugging purposes.
#[tauri::command]
pub fn get_notification_history(app: AppHandle) -> Result<String, String> {
    let _ = &app;
    let history = crate::ai::get_notification_history();
