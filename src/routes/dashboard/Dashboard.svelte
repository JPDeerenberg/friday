...
<div class="flex flex-col bg-surface-950 min-h-screen selection:bg-primary-500/30">
  ...
  <header class="sticky top-0 z-40 bg-surface-950/80 backdrop-blur-2xl border-b border-white/5 px-6 py-6 md:py-8 transition-all duration-500 overflow-hidden">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 max-w-7xl mx-auto w-full relative z-10">
      <div class="flex items-center gap-6" in:fly={{ x: -20, duration: 800 }}>
        ...
        <div>
          <h1 class="text-3xl md:text-5xl font-black text-white leading-none flex flex-wrap items-baseline gap-x-3">
          ...
          <p class="text-gray-400 text-[11px] font-bold mt-2 flex items-center gap-2">
      ...
      <div class="flex items-center gap-3 md:gap-5">
        ...
        >
          ...
          <span class="text-xs font-black text-primary-400 relative z-10">{unreadCount} berichten</span>
        </button>
        <button
        ...
        <div class="mb-6 glass rounded-m3-md p-5 border-primary-500/20 bg-gradient-to-r from-primary-500/5 to-transparent" in:fly={{ y: -10, duration: 500 }}>
          <div class="flex items-start gap-4">
            ...
            <div class="flex-1 min-w-0">
              <p class="text-[10px] font-black text-primary-400 mb-2 flex items-center gap-2">
      ...
      <div class="glass rounded-[3rem] p-6 md:p-10 relative overflow-hidden border-white/5 shadow-2xl group">
        ...
        <div class="flex items-center justify-between mb-6 md:mb-8 relative z-10">
          <h2 class="text-xl md:text-2xl font-black text-white flex items-center gap-3">
            ...
            {#if !loadingTomorrow && nextSchoolDayDate}
              <span class="text-[10px] font-black text-emerald-400/80 bg-emerald-500/10 border border-emerald-500/20 px-3 py-1.5 rounded-full not-italic ml-1">
          ...
          <button
            ...
            class="text-[10px] font-black text-emerald-400 hover:text-emerald-300 flex items-center gap-2 group/link transition-all bg-emerald-500/5 hover:bg-emerald-500/10 border border-emerald-500/10 hover:border-emerald-500/25 px-4 py-2 rounded-full"
          >
        ...
        {:else if tomorrowEvents.length === 0}
          <div class="relative z-10 flex flex-col items-center justify-center py-12 text-center" in:fade>
            ...
            <p class="text-white font-black text-base">Voorlopig geen lessen</p>
 ...
 {isOpen ? 'border-amber-500/50' : ''}"
                ...
                >
                  ...
                  <div class="flex items-center gap-4 px-5 py-4">
                    ...
                    <div class="shrink-0 flex flex-col items-center min-w-[52px]">
                      ...
                      <span class="text-[8px] font-bold text-gray-600 mt-1">
                    ...
                    <div class="shrink-0 flex items-center gap-2">
                      {#if hasHw}
                        <div class="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-500/15 border border-amber-500/25">
                          ...
                          <span class="text-[8px] font-black text-amber-400">{hw.assignments.length > 0 ? `${hw.assignments.length} taak` : 'HW'}</span>
                        </div>
                    ...
                    <div class="mx-5 mb-4 pt-3 border-t {hasHw ? 'border-amber-500/15' : 'border-white/10'} space-y-3" in:fly={{ y: -5, duration: 200 }}>
                      ...
                      {#each hw.assignments as a, j}
                        <div class="bg-white/5 rounded-m3-sm px-4 py-3 border border-white/5 {hw.isCompleted ? 'opacity-50' : ''}">
                          <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                              <p class="text-[11px] font-black text-white">{a.Titel || 'Opdracht'}</p>
                              {#if a.Omschrijving}
                                <p class="text-[10px] text-gray-400 mt-1.5 leading-relaxed line-clamp-3">{stripHtml(a.Omschrijving)}</p>
                              {/if}
                            </div>
                            {#if a.InleverenVoor}
                              <span class="shrink-0 text-[8px] font-black text-amber-400 whitespace-nowrap">
                          ...
                          <div class="flex items-center gap-2 mt-2">
                            {#if a.IngeleverdOp}
                              <span class="text-[8px] font-black text-emerald-500 bg-emerald-500/10 px-2 py-0.5 rounded-full">Ingediend</span>
                            {:else if a.Beoordeling}
                              <span class="text-[8px] font-black text-primary-400 bg-primary-500/10 px-2 py-0.5 rounded-full">Beoordeeld</span>
                            {:else}
                              <span class="text-[8px] font-black text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded-full">Open</span>
                            {/if}
                          </div>
                        </div>
                      {/each}
                      ...
                      {#if hw.isCompleted}
                        <div class="flex items-center gap-2 text-emerald-500/70 pt-1">
                          ...
                          <span class="text-[10px] font-black">Huiswerk afgerond</span>
                        </div>
                      {:else}
                        <button
                          onclick={() => markLessonDone(event, i)}
                          class="w-full flex items-center justify-center gap-2 py-2.5 rounded-m3-full bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/25 hover:text-emerald-300 transition-all text-[10px] font-black active:scale-[0.98]"
                        >
              ...
              {#if tomorrowEvents.length > lessonsBeforeBreak().length}
                <div class="flex items-center gap-3 px-5 py-3 rounded-m3-md border border-dashed border-white/10 bg-white/[0.02]">
            ...
            {#if subjectsWithHomework().length > 0}
              <div class="flex flex-wrap items-center gap-2" in:fly={{ y: 8, delay: 300, duration: 400 }}>
                <span class="text-[10px] font-black text-amber-400/70 mr-1 flex items-center gap-1.5">
                ...
                {#each subjectsWithHomework() as subject}
                  <span class="px-3 py-1.5 rounded-full bg-amber-500/15 border border-amber-500/25 text-amber-300 text-[10px] font-black">
                    {subject}
                  </span>
                {/each}
              </div>
            {:else}
              <div class="flex items-center gap-2 text-emerald-400/70" in:fly={{ y: 8, delay: 300, duration: 400 }}>
                <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
                <span class="text-[10px] font-black">Geen openstaand huiswerk voor {nextSchoolDayShortLabel()}</span>
              </div>
            {/if}
            ...
            {#if packedAndExtraHomework.length > 0}
              <div class="pt-5 border-t border-white/5 space-y-2" in:fly={{ y: 8, delay: 400, duration: 400 }}>
                <div class="flex items-center gap-2 mb-3">
                  <span class="text-[9px] font-black text-white/50">Huiswerkoverzicht — {nextSchoolDayShortLabel()}</span>
                ...
                {#each packedAndExtraHomework as item, bi (item.event.Id || bi)}
                  ...
                  <div class="rounded-m3-md border overflow-hidden transition-all {item.type === 'packed' ? 'bg-emerald-500/5 border-emerald-500/15' : 'bg-amber-500/5 border-amber-500/15'} {isExpanded ? (item.type === 'packed' ? 'border-emerald-500/30' : 'border-amber-500/30') : ''}">
                    ...
                    >
                      ...
                      <div class="flex-1 min-w-0">
                        <p class="text-[11px] font-black text-white truncate flex items-center gap-2">
                          {item.subject}
                          <span class="text-[9px] font-bold text-gray-500 normal-case not-italic">les {item.lessonHour}</span>
                        </p>
                        <p class="text-[9px] font-bold {item.type === 'packed' ? 'text-emerald-500/70' : 'text-amber-500/70'} mt-0.5">
                          {item.type === 'packed'
                            ? '✓ Ingepakt'
                        ...
                        <div class="pt-2 border-t {item.type === 'packed' ? 'border-emerald-500/10' : 'border-amber-500/10'} space-y-2.5">
                          ...
                          {#each item.hw.assignments as a}
                            <div class="bg-black/20 rounded-m3-sm px-3 py-2 {item.hw.isCompleted ? 'opacity-50' : ''}">
                              <div class="flex items-start justify-between gap-2">
                                ...
                                {#if a.InleverenVoor}
                                  <span class="shrink-0 text-[7px] font-black text-amber-400">{new Date(a.InleverenVoor).toLocaleDateString('nl-NL', { day: 'numeric', month: 'short' })}</span>
                                {/if}
                              </div>
                              {#if a.Omschrijving}
                          ...
                          {#if item.hw.isCompleted}
                            <div class="flex items-center gap-1.5 text-emerald-500/70 pt-1">
                              ...
                              <span class="text-[9px] font-black">Huiswerk afgerond</span>
                            </div>
                          {:else}
                            <button
                              onclick={() => markLessonDone(item.event, item.index)}
                              class="w-full flex items-center justify-center gap-1.5 py-2 rounded-m3-full text-[9px] font-black transition-all {item.type === 'packed' ? 'bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/25' : 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 hover:bg-emerald-500/20'} active:scale-[0.98]"
                            >
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-10">
      ...
      <div class="lg:col-span-7 space-y-8">
        <section in:fly={{ y: 30, duration: 800 }} class="space-y-3">
          <div class="flex items-center justify-between px-1 mb-4">
            ...
            <button
              onclick={() => currentPage.set('calendar')}
              class="text-[11px] font-black text-primary-400 hover:text-primary-300 transition-all hover:gap-4 flex items-center gap-3 group/all bg-primary-500/5 px-5 py-2.5 rounded-full border border-primary-500/10 hover:border-primary-500/30"
            >
          ...
          <div class="rounded-m3-md p-3 md:p-5 relative overflow-hidden group border border-white/10 bg-surface-800/50 min-h-[300px] flex flex-col">
             ...
             {:else if todayEvents.length === 0}
              <div class="flex-1 flex flex-col items-center justify-center text-center py-16" in:fade>
                ...
                <p class="text-white font-black text-base">Vrije dag!</p>
                  ...
                  >
                    <div class="flex flex-col items-center justify-center min-w-[60px] md:min-w-[75px] py-3 rounded-m3-sm bg-surface-900/80 border border-surface-700/60 group-hover/event:border-primary-500/50 transition-all shadow-md relative overflow-hidden shrink-0">
                      <div class="absolute inset-0 bg-primary-500/5 opacity-0 group-hover/event:opacity-100 transition-opacity"></div>
                       <span class="text-xl md:text-2xl font-black text-primary-400 leading-none relative z-10">{event.LesuurVan || '—'}</span>
                       <span class="text-[9px] font-bold text-gray-400 mt-1 relative z-10">{formatTime(event.Start)}</span>
                    </div>
                    <div class="flex-1 min-w-0 text-left">
                       <p class="text-base font-black text-white truncate group-hover/event:text-primary-400 transition-colors">
                        {event.Vakken?.[0]?.Naam ?? event.Omschrijving ?? 'Afspraak'}
                      </p>
                      <div class="flex flex-wrap md:flex-nowrap items-center gap-2 md:gap-5 mt-2 md:mt-4">
                         <span class="flex items-center gap-1.5 text-[10px] font-bold text-gray-300 px-3 py-1.5 rounded-m3-full bg-surface-800/80 border border-white/8">
                         ...
                         <span class="text-[10px] font-bold text-gray-400 truncate bg-white/6 px-2.5 py-1 rounded-m3-full border border-white/8">{formatTeacherName(event.Docenten?.[0]?.Naam) ?? 'Geen docent'}</span>
                      </div>
                    </div>
                    {#if event.Inhoud}
        ...
        <section in:fly={{ y: 30, delay: 200, duration: 800 }} class="space-y-4">
          <div class="flex items-center justify-between px-1 mb-4">
            ...
            <button
              ...
              class="text-[9px] md:text-[11px] font-black text-accent-400 hover:text-accent-300 md: transition-all hover:gap-3 md:hover:gap-4 flex items-center gap-2 md:gap-3 group/grades bg-accent-500/5 px-4 md:px-5 py-2 md:py-2.5 rounded-full border border-accent-500/10 hover:border-accent-500/30"
            >
            ...
            {:else}
              {#each latestGrades as grade, i (grade.Id || i)}
                ...
                >
                  ...
                  <div class="min-w-0 flex-1 relative z-10">
                    <p class="text-[13px] font-black text-gray-100 truncate leading-tight group-hover/grade:text-accent-400 transition-colors">{grade.Vak?.Omschrijving ?? 'Onbekend'}</p>
                    <p class="text-[9px] text-gray-500 font-black mt-1 truncate">{formatDate(grade.DatumIngevoerd || '')}</p>
                  </div>
                  <!-- Grade number -->
                  <div class="shrink-0 relative z-10">
                    <span class="text-3xl md:text-4xl font-black leading-none transition-transform group-hover/grade:scale-110 block {$userSettings.highlightFailing && !isVoldoende(grade) ? 'text-red-400' : 'text-accent-300'}">
                      {grade.CijferStr}
                    </span>
                  </div>
                </button>
              {:else}
                <div class="sm:col-span-2 py-10 rounded-m3-md flex flex-col items-center justify-center text-center border border-dashed border-white/15 bg-surface-800/30" in:fade>
                  ...
                  <p class="text-[12px] font-black text-gray-400">Nog geen cijfers</p>
                  <button onclick={() => currentPage.set('grades')} class="mt-3 text-[10px] text-primary-400 hover:text-primary-300 font-bold flex items-center gap-1">
      ...
      <div class="lg:col-span-5 space-y-6 md:space-y-8">
        <section in:fly={{ x: 30, delay: 400, duration: 800 }} class="space-y-6 md:space-y-10">
          <h2 class="text-xl font-black text-white flex items-center gap-3 px-1">
          ...
          <div class="glass rounded-[2.5rem] md:rounded-[4rem] p-4 md:p-8 shadow-2xl md:shadow-3xl relative overflow-hidden group border-white/5 flex flex-col min-h-[350px] md:min-h-[450px]">
            ...
            {:else if upcomingAssignments.length === 0}
              <div class="flex-1 flex flex-col items-center justify-center text-center opacity-70 py-16 md:py-24 px-4 md:px-8" in:fade>
                  ...
                  <p class="text-base md:text-lg font-black text-emerald-400/80 md: mb-4">Helemaal Bij!</p>
              </div>
            {:else}
                ...
                {#each upcomingAssignments as assignment, i (assignment.Id || i)}
                  ...
                  >
                    ...
                    <div class="min-w-0 relative z-10">
                      <p class="text-lg md:text-xl font-black text-gray-100 truncate group-hover/assign:text-red-400 transition-colors leading-tight">{assignment.Titel}</p>
                      <p class="text-[9px] md:text-[11px] text-gray-500 font-black mt-2 md:mt-3.5 md: bg-red-500/5 px-2 py-1 rounded inline-block">{assignment.Vak ?? 'Algemeen'}</p>
                    </div>
                    <div class="align-self-start sm:align-self-auto shrink-0 px-3 md:px-5 py-2 md:py-3 rounded-m3-full bg-red-500/15 border border-red-500/30 text-[10px] md:text-[12px] font-black text-red-500 shadow-xl group-hover/assign:scale-110 group-hover/assign:-rotate-2 transition-all relative z-10">
            ...
            <button
              onclick={() => currentPage.set('assignments')}
              class="w-full py-5 md:py-7 rounded-[2rem] sm:rounded-[3rem] bg-gradient-to-r from-primary-600 to-primary-400 text-white font-black text-[12px] md:text-[14px] md: shadow-2xl md:shadow-3xl shadow-primary-500/40 hover:scale-[1.03] hover:brightness-110 transition-all active:scale-95 border border-white/20 ring-[6px] md:ring-8 ring-primary-500/10 relative overflow-hidden group/btn mt-auto"
            >
        ...
        <section in:fly={{ x: 30, delay: 600, duration: 800 }} class="glass rounded-[4rem] p-12 relative overflow-hidden bg-gradient-to-br from-primary-950/60 via-surface-950 to-accent-950/60 border-l-[6px] border-primary-500/50 shadow-3xl group">
           <div class="relative z-20 flex flex-col items-center justify-center text-center py-6">
              ...
              <h3 class="text-4xl font-black text-white mb-6 leading-none animate-float">Investeer in jezelf!</h3>
              <p class="text-[13px] text-primary-200/80 font-bold max-w-[320px] leading-loose opacity-90">
